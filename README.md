...
The frontend expects the FastAPI backend API and WebSocket telemetry server to be running locally.
...
| Backend/system status | GET | `/api/status` |
| Latest telemetry packet | GET | `/api/telemetry/latest` |
| Telemetry history | GET | `/api/telemetry/history` |
| Command dispatch | POST | `/api/command` |
| Mock mission reset | POST | `/api/mock/reset?profile=demo` |
| Telemetry WebSocket stream | WS | `/ws/telemetry` |
...
- REST API requests are sent to `http://localhost:8000`.
- Live telemetry is received through `ws://localhost:8000/ws/telemetry`.
- The `RESET SESSION` button calls `POST /api/mock/reset?profile=demo`.
- The mock demo profile restarts the mission from `T+0`.

The expected demo flight sequence is:

    PRE_FLIGHT -> BOOST -> COAST -> APOGEE -> DESCENT -> LANDED

## Environment Variables

The frontend uses local defaults if no environment variables are provided.
...
If the backend is hosted somewhere else, update these values accordingly.
...
5. Click **RESET SESSION**.
6. Confirm reset.
7. Watch the mission progress through boost, coast, apogee, descent, and landing.
8. Review **Engineering Telemetry**, **Flight Analysis**, **System Logs**, and **Mission Config**.

## Data Labels

The interface uses explicit data-source labels:

- `REAL`: received directly from backend telemetry packets or backend status.
- `DERIVED`: calculated by the frontend from real packet/status values.
- `SIMULATED`: generated by the backend mock profile or used for demo/runtime clarity.
